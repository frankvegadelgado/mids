...
def string_result_format(result, count_result=False):
  ...
  Returns:
    - "Empty Graph" if result is None, "Independent Dominating Set Found a, b, c, ...." otherwise.
  """
  if result:
    if count_result:
        return f"Independent Dominating Set Size {len(result)}"
    else:
        formatted_string = f'{", ".join(f"{x + 1}" for x in result)}'
        return f"Independent Dominating Set Found {formatted_string}"
  else:
     return "Empty Graph"
